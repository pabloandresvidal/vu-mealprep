...
export async function POST(req: Request) {
  try {
    ...
    const prompt = `You are a culinary prep optimization AI. I have a multi-day meal plan involving the following recipes:
${JSON.stringify(recipes, null, 2)}

Your task is to combine all the requirements from these recipes into one seamless unified Meal Prep plan for Sunday.
1. Combine all ingredients from all recipes into a single aggregated shopping list.
...
{
  "shoppingList": [ { "item": "Chicken Breast", "combinedAmount": "4 lbs" } ],
